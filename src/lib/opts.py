...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--mask_dim', type=int, default=32,
                                 help='Number of mask coefficients.'
                                      'Must to be match with number of output channels of ProtoNet')
        self.parser.add_argument('--use_semantic_segmentation_loss', type=bool, default=True,
                                 help='Adds a 1x1 convolution directly to the biggest selected layer that predicts a '
                                      'semantic segmentations for each of the 80 classes. This branch is only '
                                      'evaluated during training time and is just there for multitask learning.')
        ...
        self.parser.add_argument('--eval_intervals', type=int, default=5,
                                 help='number of epochs to run evaluation.')
        self.parser.add_argument('--trainval', action='store_true',
                                 help='include validation in training and '
                                      'test on test set')
        self.parser.add_argument('--freeze', default='',
                                 help='backbone: freeze backbone. | all: freeze all the layers except heads.')
        ...
        self.parser.add_argument('--segm_weight', type=float, default=1,
                                 help='loss weight for semantic segmentations.')
        self.parser.add_argument('--lincomb_mask_weight', type=float, default=1,
                                 help='loss weight for lincomb_mask_loss.')
        ...
        self.parser.add_argument('--not_mask_coef', action='store_true',
                                 help='not regress local offset.')
        # exdet
    ...
    def parse(self, args=''):
        ...
        opt.mask_coef = not opt.not_mask_coef
        ...
        opt.data_dir = os.path.join('/media/reza/2TB/COCO2017', 'data')
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        if opt.task == 'exdet':
            ...
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
            if opt.mask_coef:
                opt.heads.update({'masks': opt.mask_dim})
    ...
    def init(self, args='', dataset=None):
        default_dataset_info = {
            'ctdet': {'default_resolution': [544, 544], 'num_classes': 80,
        ...
        opt = self.parse(args)
        dataset = Struct(dataset) if dataset is not None else Struct(default_dataset_info[opt.task])
