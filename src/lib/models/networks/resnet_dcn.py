...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv):
        ...
        self.mask_proto_prototype_activation = nn.ReLU(inplace=True)

        self.semantic_seg_conv = nn.Conv2d(128, heads['hm'], kernel_size=1)
    ...
    def forward(self, x):
        ...
        proto_src = x = self.deconv_layers[11](x)
        ...
        proto_out = self.proto_net(proto_src)
        ...
        ret['proto'] = proto_out
        ret['segm'] = self.semantic_seg_conv(proto_src)
        return [ret]
...
def get_pose_net(num_layers, heads, head_conv=256, freeze=None):
