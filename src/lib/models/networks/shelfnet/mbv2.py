...
# from tensorflow.contrib.fused_conv.python.ops.fused_conv2d_bias_activation_op_test_base import input_size_
...
class MobileNet2(nn.Module):
    ...
    def _make_stage(self, inplanes, outplanes, n, stride, t, stage):
        ...
        first_module = LinearBottleneck(inplanes=inplanes, outplanes=outplanes,
                                        stride=stride,
                                        t=t,
        ...
        for i in range(n - 1):
            ...
            module = LinearBottleneck(inplanes=outplanes, outplanes=outplanes,
                                      stride=1,
                                      t=6,
                                      activation=self.activation_type)
            modules[name] = module
    ...
    def _make_bottlenecks(self):
        ...
        bottleneck1 = self._make_stage(inplanes=self.c[0], outplanes=self.c[1],
                                       n=self.n[1],
                                       stride=self.s[1],
                                       t=1,
                                       stage=0)
        ...
        for i in range(1, len(self.c) - 1):
            ...
            module = self._make_stage(inplanes=self.c[i], outplanes=self.c[i + 1],
                                      n=self.n[i + 1],
                                      stride=self.s[i + 1],
                                      t=self.t,
                                      stage=i)
    ...
    def forward(self, x):
        ...
        c1 = x = self.bottlenecks.Bottlenecks_1(x)
        c2 = x = self.bottlenecks.Bottlenecks_2(x)
        x = self.bottlenecks.Bottlenecks_3(x)
        c3 = x = self.bottlenecks.Bottlenecks_4(x)
        x = self.bottlenecks.Bottlenecks_5(x)
        c4 = x = self.bottlenecks.Bottlenecks_6(x)
        # x = self.conv_last(x)
        # x = self.bn_last(x)
        # x = self.activation(x)
        ...
        return c1, c2, c3, c4  # F.log_softmax(x, dim=1) #TODO not needed(?)
...
if __name__ == "__main__":
    ...
    model = MobileNet2(input_size=224, activation=nn.ReLU)
    img = Variable(torch.ones(1, 3, 224, 224))
    predict = model(img)
