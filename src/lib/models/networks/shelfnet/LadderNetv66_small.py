...
class Initial_LadderBlock(nn.Module):
    ...
    def __init__(self, planes, layers, kernel=3, block=BasicBlock, inplanes = 3):
        ...
        for i in range(0,layers):
            self.down_conv_list.append(nn.Conv2d(planes*2**i, planes*2**(i+1), stride=2,
        ...
        for i in range(0, layers):
            self.up_conv_list.append(nn.ConvTranspose2d(in_channels=planes*2**(layers-i),
                                                        out_channels=planes*2**max(0,layers-i-1), kernel_size=3,
...
class Decoder(nn.Module):
    def __init__(self, planes, layers, kernel=3, block=BasicBlock):
        ...
        self.kernel = kernel
        self.padding = int((kernel - 1) / 2)
        self.inconv = block(planes[0], planes[0])

        # create module for bottom block
        self.bottom = block(planes[layers-1], planes[layers-1])
        ...
        for i in range(0, layers-1):
            self.up_conv_list.append(
                nn.ConvTranspose2d(planes[layers - 1 - i], planes[max(0, layers - i - 2)],
                                   kernel_size=3,
                                   stride=2,
                                   padding=1,
                                   output_padding=1,
                                   bias=True))
            self.up_dense_list.append(block(planes[max(0, layers - i - 2)], planes[max(0, layers - i - 2)]))
    ...
    def forward(self, x):
        ...
        for j in range(0, self.layers-1):
            o1 = self.up_conv_list[j](out)
            o2 = x[self.layers - j - 2]
            out = o1 + o2
            # out = F.relu(out)
            out = self.up_dense_list[j](out)
            up_out.append(out)
...
class LadderBlock(nn.Module):
    ...
    def __init__(self, planes, layers, kernel=3, block=BasicBlock):
        ...
        self.down_module_list = nn.ModuleList()
        for i in range(0, layers-1):
            self.down_module_list.append(block(planes[i], planes[i]))

        # use strided conv instead of pooling
        self.down_conv_list = nn.ModuleList()
        for i in range(0, layers-1):
            self.down_conv_list.append(nn.Conv2d(planes[i], planes[i+1],
                                                 stride=2,
                                                 kernel_size=kernel,
        ...
        self.up_dense_list = nn.ModuleList()
        for i in range(0, layers-1):
            self.up_conv_list.append(nn.ConvTranspose2d(planes[layers-i-1], planes[max(0, layers-i-2)],
                                                        kernel_size=3,
                                                        stride=2,
                                                        padding=1,
                                                        output_padding=1,
    ...
    def forward(self, x):
        ...
        for j in range(0, self.layers - 1):
            out1 = self.up_conv_list[j](out)
            out2 = down_out[self.layers - j - 2]
            out = out1 + out2
            # out = F.relu(out)
            out = self.up_dense_list[j](out)
            up_out.append(out)
